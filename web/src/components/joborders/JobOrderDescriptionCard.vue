      ...
      <dashboard-card-widget>
        <template v-slot:header>
          <div class="flex justify-between">
            <p class="p-3 font-semibold text-lg">Description</p>
        ...
        <template v-slot:body>
          <div class="col-span-1 flex flex-col p-2 border-t-2 mr-10">
            ...
            <div v-else class="sm:col-span-1 md:flex lg:flex p-2">
              <div v-html="data.description" class="prose"></div>
            </div>
          </div>
        </template>
      </dashboard-card-widget>
...
import { updateJoborderDescription } from "../../services";
...
export default defineComponent({
  ...
  methods: {
    async saveChanges() {
      this.editMode = false;
      this.loading = true;
      const { data, errors } = await updateJoborderDescription({
